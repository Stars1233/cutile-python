...
"""fixed dense BF16 FlashAttention forward kernel for Blackwell."""
...
def tcgen05_load16_vector(address):
...
@cl.kernel(
    ...
    min_blocks_per_sm=1,
    _ptx_compiler_verbose=True,
    _ptx_compiler_warn_on_local_memory_usage=True,
    _ptx_compiler_warn_on_spills=True,
)
def flash_attention_fwd_kernel(
    ...
    # Keep these operations on the same control-flow branches as their roles.
    # The 96 + 176 + 176 + 64 targets consume the 512-register CTA pool.
    if warpgroup == 3:
    ...
    if warp == 15 and cl.elect_sync():
        ...
        while current_bid < total_bids:
            ...
            for key_block in range(iterations):
                cl.mbarrier_wait_parity(
                    kv_finished.get_element_pointer(kv_index), kv_phase
                )
        ...
        while current_bid < total_bids:
            ...
            for key_block in range(iterations):
                ...
                cl.mbarrier_wait_parity(
                    kv_arrived.get_element_pointer(v_stage), kv_phase
                )
                ...
                if key_block + 1 < iterations:
                    ...
                    cl.mbarrier_wait_parity(
                        kv_arrived.get_element_pointer(k_stage), kv_phase
                    )
                    cl.tcgen05_fence_after_thread_sync()
                    for qid in cl.static_iter(range(2)):
                        score_tmem = cl.tcgen05_tmem_offset(
        ...
        cl.setmaxregister_increase(176)
        qid = warpgroup
        score_phase = 0
        rescale_phase = 1
        current_bid = cl.block_index(0)
        while current_bid < total_bids:
            ...
            cl.mbarrier_wait_parity(
                rescale_finished.get_element_pointer(qid), rescale_phase
            )
            rescale_phase ^= 1

            for key_block in range(iterations):
                cl.mbarrier_wait_parity(
                    scores_arrived.get_element_pointer(qid), score_phase
                )
                ...
                for scores in cl.static_iter(score_quarters):
                ...
                for quarter in cl.static_iter(range(4)):
                    probabilities, quarter_sum = probability_vector(
                        score_quarters[quarter], scale, offset
                    )
                    ...
                    cl.tcgen05_store(
                        cl.Tcgen05LoadStoreShape.SHAPE_32X32B,
                        cl.tcgen05_tmem_offset(row_tmem, column_offset=quarter * 16),
                        probabilities,
                    )
                ...
                for qid in cl.static_iter(range(2)):
                    ...
                    if needs_rescale:
                        for column in cl.static_iter(range(0, HEAD_DIM_V, 16)):
                            ...
                            values = tcgen05_load16(output_row)
                            cl.tcgen05_wait_load()
                            cl.tcgen05_store(
                                cl.Tcgen05LoadStoreShape.SHAPE_32X32B,
                                output_row,
...
def test_bf16_b200_flash_attention_fwd(batch, q_sequence, kv_sequence, heads):
    torch.manual_seed(0)
    q = torch.randn(
    ...
    actual, actual_lse = run_flash_attention(q, k, v)
    torch.cuda.synchronize()
    expected, expected_lse = reference_attention(q, k, v)
...
if __name__ == "__main__":
    test_bf16_b200_flash_attention_fwd(1, 512, 512, 1)
    test_bf16_b200_flash_attention_fwd(1, 512, 1024, 1)
    test_bf16_b200_flash_attention_fwd(2, 512, 512, 2)
    print("success")
